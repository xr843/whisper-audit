...
- 返回的 `spk_embedding` 是 **CUDA tensor**，不是 numpy：直接 `np.asarray()`
  ...
  `.detach().cpu().numpy()`。
- **同一个人**的 30 段真实录音（`scratchpad/smoke.wav`，5 分钟，经文朗读与
  白话讲解两种腔调交替），两两余弦距离中位数 0.70、90 分位 0.94、最大
  1.02，average-linkage 把这 30 段合成一簇的最终合并距离实测 0.9011——
  比 ModelScope 模型卡片给的说话人确认经验阈值（`thr=0.31` 相似度，换算
  成距离约 0.69）宽松得多。照抄 0.69 会把这份单人录音硬拆成 3 簇（实测
  过）。诵经与讲解交替造成的韵律差异，把同一人的声纹在向量空间里冲得
  很散，说话人确认场景（干净的单一语气朗读）校准出来的阈值不能直接照搬
  到这类内容。`DEFAULT_THRESHOLD` 按这份真实数据校准，见下方注释。
...
# 真机校准（见模块 docstring）：单人录音（30 段，诵经+讲解混杂腔调）里，
# average-linkage 把全部段合成一簇所需的最终合并距离实测 0.9011。
# 0.95 在此基础上留出安全边际，保证"不能把单人拆开"这条硬要求。
#
# 这是按"不能误拆单一说话人"校准的下限，**不是**按"能可靠分开两个人"校准的——
# 没有真实多说话人录音可供验证，这份阈值分离真实说话人的能力未经验证，
# 见 diarize-report.md 的顾虑一节。n_speakers 已知时应显式传入，不依赖自动定数。
